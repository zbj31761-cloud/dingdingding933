...
export const EventModal: React.FC<EventModalProps> = ({
  ...
  useEffect(() => {
    ...
    if (parts.length === 3) {
      ...
      if (!isNaN(month)) setCurrentMonth(month);
  ...
  const handleConfirm = () => {
    const finalTitle = description.trim() || `日常日程 (${selectedTime})`;
    ...
    onConfirm({
      title: finalTitle,
      time: selectedTime,
      locationOrType: selectedReminder !== '不提醒' ? selectedReminder : '日常日程',
      dateStr,
    });
    setDescription('');
    onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/20 backdrop-blur-sm animate-in fade-in duration-200 select-none">
      ...
      >
        ...
        <div className="glass-panel-soft rounded-2xl p-3.5 border border-white/80 mt-1">
          <input
            type="text"
            id="input-modal-event-desc"
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            placeholder="请输入日程描述 (如：团队同步例会)"
            autoFocus
        ...
        <div className="relative">
          <div className="glass-panel-soft rounded-2xl py-2.5 px-4 flex items-center justify-between text-xs font-medium text-slate-700 border border-white/80">
            ...
            <button
              type="button"
              id="btn-toggle-event-time-picker"
              onClick={() => {
            ...
            <button
              type="button"
              id="btn-toggle-event-reminder-picker"
              onClick={() => {
